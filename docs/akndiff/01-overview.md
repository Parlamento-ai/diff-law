...
AKN Diff fills this gap with two elements: `changeSet` and `vote`.

## Namespace

AKN Diff uses its own XML namespace so that standard AKN processors can safely ignore it:

| Standard | Namespace URI |
|---|---|
| Akoma Ntoso 3.0 | `http://docs.oasis-open.org/legaldocml/ns/akn/3.0` |
| **AKN Diff** | `http://parlamento.ai/ns/akndiff/1.0` |
...
- **Non-invasive**: AKN Diff elements live alongside standard AKN elements. An AKN processor that doesn't know about the `akndiff` namespace will simply skip them. The document remains valid AKN.
